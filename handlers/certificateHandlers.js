...
function buildCertificatePdf(data, txnId) {
 return new Promise((resolve, reject) => {
   ...
   const description = data.description || data.subCategory || 'คาใช้จ่ายตามสลิป';

   // ชื่อเรื่องกลางหน้า
   doc.font(fontBoldPath).fontSize(24)
   ...
   doc.text('ผซื้อ/ผู้รับบริการ: ฟูริน มัทฉะ', 60, infoY);
   ...
   // วนที่ชิดขวา
   doc.text(`วนที่: ${thaiDate}`, 60, infoY + 95, { align: 'right', width: 475 });

   const tableTop = infoY + 125;
   ...
   doc.text('ลำดับ', col1x, tableTop + 7, { width: col2x - col1x, align: 'center' });
   doc.text('รายละเอียด', col2x, tableTop + 7, { width: col3x - col2x, align: 'center' });
   doc.text('จำนวนเงิน (บาท)', col3x, tableTop + 7, { width: col4x - col3x, align: 'center' });
   doc.text('หมายเหตุ', col4x, tableTop + 7, { width: tableRight - col4x, align: 'center' });
   ...
   doc.moveDown(0.8);
   doc.text(`ข้าพเจ้า ${userLogger} (ผู้เบกจ่าย)`);
   doc.moveDown(0.5);
   doc.text(
   ...
   doc.text(`(${userLogger})`, sign1X, signY + 20, { width: 200, align: 'center' });
   doc.text(`(${data.approverName || userLogger})`, sign2X, signY + 20, { width: 200, align: 'center' });

   doc.font(fontBoldPath).fontSize(13);
   doc.text('ผู้เบิกจ่าย', sign1X, signY + 40, { width: 200, align: 'center' });
   doc.text('ผู้อนุมัติ', sign2X, signY + 40, { width: 200, align: 'center' });
