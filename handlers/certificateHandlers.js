...
function buildCertificatePdf(data, txnId) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.5);
    doc.text(
      `ขอรับรองว่า รายจ่ายข้างต้นนี้ไม่อาจเรียกเก็บใบเสร็จรับเงินจากผู้รับได้ และขาพเจ้าได้จ่ายไปในงานของทาง ร้านค้า/กิจการเจ้าของคนเดียว โดยแท้ ตั้งแต่วนที่ ${data.date} ถึงวันที่ ${data.date}`,
      { width: 475, lineGap: 2 }
    );
